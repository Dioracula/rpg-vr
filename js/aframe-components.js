// js/aframe-components.js

// Efeito Visual de Rastro
AFRAME.registerComponent('efeito-rastro', {
    schema: { 
        alvoId: { type: 'string' },
        offsetZ: { type: 'number', default: -0.8 },
        angleZ: { type: 'number', default: 0 },
        rotX: { type: 'number', default: 0 },
        rotY: { type: 'number', default: 0 }
    },
    init: function() {
        this.alvo = document.querySelector(this.data.alvoId);
        this.cam = document.querySelector('[camera]');
    },
    tick: function() {
        if (!this.alvo || !this.cam) return;
        
        let pos = new THREE.Vector3(0, 0, this.data.offsetZ);
        pos.applyMatrix4(this.alvo.object3D.matrixWorld);
        this.el.object3D.position.copy(pos);

        this.el.object3D.quaternion.copy(this.cam.object3D.getWorldQuaternion(new THREE.Quaternion()));
        
        this.el.object3D.rotateX(THREE.MathUtils.degToRad(this.data.rotX));
        this.el.object3D.rotateY(THREE.MathUtils.degToRad(this.data.rotY));
        this.el.object3D.rotateZ(this.data.angleZ);
    }
});

// Botões do Sistema (Menu UI VR/PC)
...
AFRAME.registerComponent('colisor-arma-vr', {
    ...
    tick: function(time, timeDelta) {
        ...
        if (this.cortando) {
            if (time - this.tempoCorte > 400) {
                ...
                if (armaStats.categoria === 'Espada') {
                    ...
                    posPonta.add(dirPonta.multiplyScalar(0.6)); 
                }
        ...
        // Bounding Box Exata da Espada
        let boxArma = new THREE.Box3();
        let visualEl = this.el.querySelector(this.data.mao === 'direita' ? '#arma-visual-dir' : '#arma-visual-esq');
        if (visualEl) {
            visualEl.object3D.updateMatrixWorld(true);
            boxArma.setFromObject(visualEl.object3D);
        }
        if (boxArma.isEmpty()) { 
            boxArma.setFromCenterAndSize(currentWorldPos, new THREE.Vector3(0.2, 0.2, 0.2));
        }
        ...
        inimigosEls.forEach(inimigoEl => {
            ...
            if(syncComp && syncComp.hpAtual > 0) {
                ...
                if(colisorNode) {
                    ...
                    boxInimigo.setFromObject(colisorNode.object3D);
                } else {
                    inimigoEl.object3D.updateMatrixWorld(true);
                    boxInimigo.setFromObject(inimigoEl.object3D);
                }
...
// Defende e Ricocheteia Projéteis
AFRAME.registerComponent('projetil-inimigo-fisico', {
    ...
    tick: function (time, timeDelta) {
        ...
        if (this.el.dataset.defletido !== "true") {
            ...
            let maoDir = document.querySelector('#mao-direita');
            if (maoEsq) maos.push({el: maoEsq, esq: true});
            if (maoDir) maos.push({el: maoDir, esq: false});

            for (let i = 0; i < maos.length; i++) {
                let mao = maos[i].el;
                let isEsq = maos[i].esq;
                ...
                if (isEscudo || isLuva || isEspada) {
                    let posMao = new THREE.Vector3();
                    mao.object3D.getWorldPosition(posMao);
                    
                    let raioColisao = isEspada ? 0.8 : 0.4;
                    if (isEscudo) raioColisao = 0.5;
                    
                    if (posProj.distanceTo(posMao) < raioColisao) {
                        let colisorComp = mao.components['colisor-arma-vr'];
                        let handSpeed = colisorComp ? colisorComp.speed : 0;
                        
                        if (isEscudo) {
                            if (handSpeed > 1.5) {
                                this.el.dataset.defletido = "true";
                                let handVel = colisorComp ? colisorComp.vel.clone().normalize() : new THREE.Vector3(0, 1, -1).normalize();
                                if (handVel.lengthSq() === 0) handVel.set(0, 1, -1).normalize();
                                this.vel.copy(handVel).multiplyScalar(20); 
                                window.tocarSom('snd-sword');
                                return; 
                            } else {
                        ...
                        } else if (isEspada || isLuva) {
                            if (handSpeed > 1.5) {
                                if(isEspada) {
                                    window.tocarSom('snd-sword'); 
                                    this.el.remove();
                                    window.gerarParticulaRastro(posProj, null, '#FFFFFF');
                                    return; 
                                } else if (isLuva) {
                                    this.el.dataset.defletido = "true";
                                    let handVel = colisorComp ? colisorComp.vel.clone().normalize() : new THREE.Vector3(0, 1, -1).normalize();
...
AFRAME.registerComponent('vr-magia-lancar', {
    init: function() {
        ...
        this.el.addEventListener('triggerdown', () => {
            ...
            this.gravando = true;
            this.posInicio = new THREE.Vector3(0, 0, -0.45);
            this.posInicio.applyMatrix4(this.el.object3D.matrixWorld);
            this.tempoInicio = Date.now();
            
            const tipVisEl = this.el.querySelector('#wand-tip-vis');
            if (tipVisEl) {
                tipVisEl.setAttribute('material', 'emissive: #fff; emissiveIntensity: 1.5'); 
            }
        });
        ...
        this.el.addEventListener('triggerup', () => {
            ...
            const tipVisEl = this.el.querySelector('#wand-tip-vis');
            if (tipVisEl) {
                tipVisEl.setAttribute('material', 'emissive: #00FFFF; emissiveIntensity: 0.8'); 
            }
            
            let posFim = new THREE.Vector3(0, 0, -0.45);
            posFim.applyMatrix4(this.el.object3D.matrixWorld);
            ...
            if (tempoGasto > 50 && deltaWorld.length() > 0.15) {
                ...
                if (isHorizontal) {
                    corMagia = '#9b59b6'; custoMP = 10; multDano = 1.0; scaleMagia = '1 1 1';
                } else {
                    corMagia = '#00FFFF'; custoMP = 15; multDano = 1.5; scaleMagia = '1.5 1.5 1.5';
                }
                ...
                let quatMao = new THREE.Quaternion();
                this.el.object3D.getWorldQuaternion(quatMao);
                let castDir = new THREE.Vector3(0, 0, -1);
                castDir.applyQuaternion(quatMao).normalize();
                ...
                if (armaStats.projetilGlb && armaStats.projetilGlb.trim() !== '') {
                    ...
                    proj.innerHTML = `
                        <a-sphere radius="0.1" color="${corMagia}" material="emissive: ${corMagia}; emissiveIntensity: 1"></a-sphere>
                        <a-light type="point" color="${corMagia}" intensity="0.5" distance="3"></a-light>
                    `;
                }
...
// Projétil Físico (Shuriken / Flecha)
...
AFRAME.registerComponent('sync-cintura', {
    ...
    tick: function() {
        ...
        if (window.playerState.shurikens > 0) {
            this.visualDir.setAttribute('visible', 'true');
        } else {
            this.visualDir.setAttribute('visible', 'false');
        }
    }
});
    ...
    init: function() {
        ...
        this.el.addEventListener('gripdown', () => {
            ...
            for(let i=0; i<shurikensChao.length; i++) {
                let sChao = shurikensChao[i];
                let posS = new THREE.Vector3(); sChao.object3D.getWorldPosition(posS);
                if (posMao.distanceTo(posS) < 0.4) {
                    this.origemGrab = sChao; 
                    sChao.parentNode.removeChild(sChao); 
                    pegouChao = true;
                    break;
                }
            }
            ...
            if (!pegouChao && window.playerState.shurikens > 0) {
                ...
                if (cintura) {
                    ...
                    let offsetDir = new THREE.Vector3(0.25, -0.1, 0.1);
                    offsetDir.applyQuaternion(cintura.object3D.quaternion);
                    posCintura.add(offsetDir);
                    
                    if (posMao.distanceTo(posCintura) < 0.35) {
                        this.origemGrab = 'cinto';
                        pegouCinto = true;
                    }
                }
            }
            ...
            if (pegouChao || pegouCinto) {
                ...
                if(!this.visualInHand) {
                    this.visualInHand = document.createElement('a-entity');
                    this.visualInHand.innerHTML = window.bancoDeArmas['Shuriken'].visualDir;
                    this.el.appendChild(this.visualInHand);
                }
        ...
        this.el.addEventListener('gripup', () => {
            if (this.segurando) {
                ...
                if (speed > 1.5) {
                    ...
                    inimigosAtuais.forEach(ini => {
                        ...
                        if(syncComp && syncComp.hpAtual > 0) {
                            ...
                            let dirMao = vel.clone().normalize();
                            let dirToIni = new THREE.Vector3().subVectors(iniPos, currPos).normalize();
                    ...
                    if (bestTarget) {
                        let forcaReal = vel.length();
                        let dirAtual = vel.normalize();
                        dirAtual.lerp(bestTarget, 0.35).normalize();
                        vel = dirAtual.multiplyScalar(forcaReal);
                    }
                } else {
                    vel.set(0, 0, 0); 
                }
                ...
                let proj = document.createElement('a-entity');
                proj.setAttribute('position', `${currPos.x} ${currPos.y} ${currPos.z}`);
                
                let projVis = document.createElement('a-entity');
                projVis.innerHTML = window.bancoDeArmas['Shuriken'].visualDir;
                proj.appendChild(projVis);

                let armaStats = window.bancoDeArmas['Shuriken'];
                let danoTiro = window.playerState.forca + (armaStats.danoBonus || 5);
